...
class SingleRegion extends EventEmitter<RegionEvents> {
  ...
  constructor(params: RegionParams, private totalDuration: number) {
    ...
    this.splitChannels = params.channel === undefined ? false : true;
  ...
  private initElement(content?: string | HTMLElement) {
    ...
    const height = this.splitChannels ? '50%' : '100%';
    const top = this.channel ? '50%' : '0';
  ...
  public _onUpdate(dx: number, side?: 'start' | 'end') {
    ...
    const deltaSeconds =
      (dx / this.element.parentElement.clientWidth) * this.totalDuration;
    const newStart =
      !side || side === 'start' ? this.start + deltaSeconds : this.start;
  ...
  public setOptions(options: {
    ...
    if (options.resize !== undefined) {
      ...
      this.element.querySelectorAll('[data-resize]').forEach((handle) => {
        (handle as HTMLElement).style.cursor = this.resize
          ? 'ew-resize'
          : 'default';
      });
    }
  ...
  onInit() {
    ...
    this.subscriptions.push(
      this.wavesurfer.on('timeupdate', (currentTime) => {
        const playedRegion = this.regions.find(
          (region) => region.start <= currentTime && region.end >= currentTime
        );
